...
mkdir -p "$root/tmp"
jq -n --arg total "$rows_total" --arg pass "$rows_pass" --arg fail "$rows_fail" --arg promo "$rows_promo" '{eval_total:($total|tonumber),eval_pass:($pass|tonumber),eval_fail:($fail|tonumber),promotions:($promo|tonumber)}' > "$root/tmp/skill-sql-cert-summary.json"
