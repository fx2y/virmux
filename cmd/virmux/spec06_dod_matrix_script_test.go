...
func TestSpec06DoDMatrixFailsWithoutParallelProof(t *testing.T) {
  ...
  "reports_count": 1
	...
	mustWriteScriptFixtureFile(t, filepath.Join(repo, "tmp", "research-cert.ok"), "{\"cert_ts\":\"2026-02-24T00:00:00Z\"}\n")
	mustWriteScriptFixtureFile(t, filepath.Join(repo, "tmp", "research-portability.ok"), "{\"cert_ts\":\"2026-02-24T00:00:00Z\"}\n")
	mustWriteScriptFixtureFile(t, filepath.Join(repo, "tmp", "research-docs-drift.ok"), "{\"cert_ts\":\"2026-02-24T00:00:00Z\"}\n")
...
func TestSpec06DoDMatrixFailsOnStaleProofs(t *testing.T) {
	t.Parallel()
	repo := newScriptTestRepo(t)
  ...
  "reports_count": 1
	...
	mustWriteScriptFixtureFile(t, filepath.Join(repo, "tmp", "research-cert.ok"), "{\"cert_ts\":\"2026-02-24T00:00:00Z\"}\n")
	mustWriteScriptFixtureFile(t, filepath.Join(repo, "tmp", "research-parallel.ok"), "{\"cert_ts\":\"2026-02-24T00:00:00Z\"}\n")
	mustWriteScriptFixtureFile(t, filepath.Join(repo, "tmp", "research-portability.ok"), "{\"cert_ts\":\"2026-02-24T00:00:00Z\"}\n")
	mustWriteScriptFixtureFile(t, filepath.Join(repo, "tmp", "research-docs-drift.ok"), "{\"cert_ts\":\"2026-02-24T00:00:00Z\"}\n")

	out, err := runScriptFromRepo(t, repo, "spec06_dod_matrix.sh", "--db", dbPath, "--cert-ts", "2026-02-24T00:00:01Z")
	if err == nil {
		t.Fatalf("expected stale-proof failure, output=\n%s", out)
	}
	if !strings.Contains(out, "stale or mismatched research-sql-cert-summary.json") {
		t.Fatalf("expected stale summary error, output=\n%s", out)
	}
}
